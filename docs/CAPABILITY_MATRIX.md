...
|---|---|---|---|---|
| 1. Ground segmentation | Implemented | Percentile-grid ground heuristic | services/ml/pipeline/ground_classification.py | Operational heuristic; not Cloth Simulation Filter (CSF). |
...
| 5b. PointNet++ wood/leaf candidate | Experimental | PointNet++ backend exists but is not the promoted default | services/ml/pipeline/pointnet_inference.py; docs/ml/WOODLEAF_RESULTS.md | Wan held-out Wood IoU 0.418 and Mean IoU 0.613, with selection/provenance limitations; not production-promoted. |
| 6. QSM-derived geometry | Implemented | RANSAC circle DBH, maximum-Z height, and taper/stacked-cylinder stem volume; branch volume is zero | services/ml/pipeline/qsm.py | Operational geometry estimate with known volume limitations; not full TreeQSM branch-axis modelling. |
...
| 8. Allometric carbon calculation | Implemented | species_db.csv species-specific equations with Chave 2014 pantropical fallback | services/ml/pipeline/allometric.py; services/ml/data/species_db.csv | Produces biomass, carbon stock, and CO2e estimates; coefficient verification against TGO 2017 remains required. |
...
| Async analysis jobs | Implemented | FastAPI jobs endpoints, persistent job state, and a separate worker process | services/api/app/api/v1/jobs.py; services/api/app/worker.py | Implemented and covered by API tests; production hosting is separate. |
