...
| 8. Allometric carbon calculation | Implemented | Chave 2014 pantropical, at the named species' wood density when one is given; the species-specific equations in species_db.csv are gated off until their coefficients are checked against the cited papers | services/ml/pipeline/allometric.py; services/ml/data/species_db.csv; services/ml/pipeline/tver.py; docs/evidence/cameroon_61/result.json | Produces biomass, carbon stock, and CO2e estimates. TGO's own table (T-VER-S-TOOL-01-01 v2 Appendix 2) is transcribed and scored against 61 destructively harvested tropical trees, and one of its rows predicts more mass than solid wood; the species-specific equations in species_db.csv remain gated off pending checks against their own cited papers. |
