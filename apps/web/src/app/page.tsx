...
const JOURNEY = [
  ...
  },
  {
    step: '03',
    title: 'วัด geometry',
    description: 'หา DBH ความสูง และปริมาตรจากโครงสร้างของต้นไม้ โดยรายงานขอบเขต validation แยกจากกัน',
  },
  {
    step: '04',
    title: 'ประมาณ carbon stock',
    description: 'แปลง geometry ผ่าน allometric equation พร้อม provenance; species classification ยังเป็น Stub',
  },
] as const;

export default function HomePage() {
  return (
    <main className="min-h-screen overflow-x-hidden bg-gallery-ivory text-forest-ink">
        ...
        <div className="grid grid-cols-1 gap-8 lg:min-h-[35.625rem] lg:grid-cols-12">
          <div className="flex flex-col justify-center lg:col-span-6 lg:pr-8">
            ...
            <p className="mt-7 max-w-xl text-base leading-8 text-canopy sm:text-lg">
              แพลตฟอร์มต้นแบบประเมิน DBH ความสูง ชีวมวล คาร์บอน และ CO₂e จาก 3D point cloud
              พร้อม provenance ของ pipeline ที่ตรวจสอบย้อนกลับได้
            </p>
            ...
            <p className="mt-6 font-mono text-[0.625rem] uppercase leading-5 tracking-[0.08em] text-evidence-amber">
              {baseline.backend} = Default · {candidate.displayName} = {candidate.status} · species stage = Stub
            </p>
          </div>
          ...
          <div className="relative min-h-[25rem] overflow-hidden rounded-[1.75rem] shadow-[0_28px_68px_-18px_rgba(14,42,29,0.18)] lg:col-span-6 lg:min-h-[35.625rem]">
            ...
            <div className="absolute bottom-6 right-6 max-w-[16rem] rounded-[0.875rem] border border-moss bg-deep-forest/90 px-4 py-3 text-paper backdrop-blur-sm">
              <p className="font-mono text-[0.5625rem] uppercase tracking-[0.08em]">Field observation / 01</p>
        ...
        >
          <div className="grid items-end gap-8 lg:grid-cols-12">
            ...
            <div className="border-l border-moss pl-5 text-sm leading-7 text-canopy lg:col-span-4">
              ขอบเขต prototype ยังไม่รวม mobile photogrammetry ที่ผ่านการ review, marketplace หรือ certification;
              ค่าที่แสดงเป็น carbon stock และ CO₂e estimate ไม่ใช่ certified credit
            </div>
          </div>
        </EditorialSection>
      </div>
        ...
        >
          <div className="overflow-hidden rounded-[1.75rem] bg-deep-forest text-paper shadow-[0_24px_60px_-24px_rgba(14,42,29,0.45)]">
            <div className="grid lg:grid-cols-12">
              <div className="relative min-h-[20rem] overflow-hidden border-b border-paper/15 lg:col-span-7 lg:border-b-0 lg:border-r">
                ...
                <div className="absolute left-6 top-6 rounded-full border border-lichen/40 bg-deep-forest/80 px-3 py-1 font-mono text-[0.625rem] uppercase tracking-[0.1em] text-lichen">
              ...
              <dl className="space-y-0 lg:col-span-5">
                <div className="border-b border-paper/15 p-6">
                  <dt className="font-mono text-[0.625rem] uppercase tracking-[0.12em] text-lichen">Default backend</dt>
                  <dd className="mt-2 font-display text-3xl">{baseline.backend}</dd>
                  <p className="mt-2 text-sm leading-6 text-mist">Implemented baseline และเส้นทางหลักของ demo</p>
                </div>
                <div className="border-b border-paper/15 p-6">
                  <dt className="font-mono text-[0.625rem] uppercase tracking-[0.12em] text-evidence-amber">Candidate status</dt>
                  <dd className="mt-2 font-display text-3xl">{candidate.displayName} / {candidate.status}</dd>
                  <p className="mt-2 text-sm leading-6 text-mist">ไม่ถูก promote เป็น default จากผลที่เห็นบนหน้านี้</p>
                </div>
                <div className="p-6">
                  <dt className="font-mono text-[0.625rem] uppercase tracking-[0.12em] text-lichen">Reproducible core demo</dt>
                  <dd className="mt-2 font-display text-3xl tabular-nums">{coreDemo.totalTrees} trees</dd>
                  <p className="mt-2 text-sm leading-6 text-mist">ค่าจาก deterministic demo ไม่ใช่ accuracy benchmark</p>
                </div>
              </dl>
            </div>
          </div>
        ...
        >
          ...
          <div className="mt-8 flex flex-col justify-between gap-6 border-t border-hairline pt-8 sm:flex-row sm:items-center">
            <p className="max-w-3xl text-sm leading-7 text-canopy">
              Species classifier = Stub. ค่า carbon stock และ CO₂e เป็น estimate; coefficients ยังต้อง verify กับ
              TGO Guideline 2017 และไม่มี claim เรื่อง marketplace หรือการรับรองเครดิต
            </p>
            <Button render={<Link href="/demo" />} variant="editorial" size="xl">
              เปิดหลักฐานใน Demo
            </Button>
